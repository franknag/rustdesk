                    ...
                    Some(rendezvous_message::Union::PeerDiscovery(p)) => {
                        ...
                        if p.cmd == "pong" {
                            ...
                            if local_mac.is_empty() && p.mac.is_empty() || local_mac != p.mac {
                                if !hbb_common::config::PeerConfig::exists(addr.ip().to_owned()) { // Add direct connection peer
                                    hbb_common::config::PeerConfig::store(hbb_common::config::PeerConfig.default(), addr.ip().to_owned());
                                }
